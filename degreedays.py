...
def get_data(infilename: str) -> dict[str, dict[str, int]]:
    ...
    with open(infilename, 'r') as f:
        ...
        for row in reader:
            #print(row)
            if row['Date'][0].isdigit():
                data[row['Date']] = {'heating': int(row['HDD']),
                                     'cooling': int(row['CDD'])}
    return data
...
def make_degree_day_dict(entries: list[tuple[str, int, int]]) -> dict[str, dict[str, int]]:
...
def write_to_file(outfilename: str, totals: dict[str, int]) -> None:
